...
async def test_start_preserves_headless_and_launch_args(
    tmp_path, headless, monkeypatch
):
    monkeypatch.setattr(
        "linkedin_mcp_server.core.browser.hidden_target_is_supported", lambda: False
    )
    context = MagicMock()
    context.pages = []
